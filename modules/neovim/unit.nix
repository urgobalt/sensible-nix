  ...
  sensibleLib.sensibleConfig {
    ...
    home.home.packages = with pkgs;
      lib.flatten [
        ...
        (f "python"      [python312Packages.python-lsp-server python312Packages.pylsp-rope])
        (f "html-css-js" [vscode-langservers-extracted])
        (f "tailwindcss" [tailwindcss tailwindcss-language-server])
        (f "c"           [clang-tools])
        (f "lua"         [lua-language-server stylua])
        (f "nix"         [nil alejandra])
      ];
  }
